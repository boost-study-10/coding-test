...
![](https://velog.velcdn.com/images/seha01130/post/3ed1b8db-064b-47d3-9154-553c8d55645e/image.png)
...
![](https://velog.velcdn.com/images/seha01130/post/92d35572-b3f5-40bf-ba1d-400839127913/image.png)

## ⏰ 시간복잡도
`O(N^2)`
알고리즘의 시간 복잡도를 계산할 때는 단순히 루프의 개수만 세는 게 아니라, **가장 안쪽의 코드가 총 몇 번 실행되는가**를 봐야함

>
- 바깥쪽 루프 ($i$): $0$부터 $N/2$까지 (약 $\frac{N}{2}$번)
- 중간 루프 ($k$): 항상 4번 (상수)
- 안쪽 루프 ($j$): $2, 4, 6, 8, \dots, N-1$까지 증가
>
이걸 다 더하면 결국 **"표(배열)의 모든 칸을 채우는 횟수"** 가 된다.
$N \times N$ 크기의 표에서 가운데 1을 제외한 나머지 칸들을 한 번씩만 방문해서 숫자를 채움 -> 가장 안쪽에 있는 `arr[currentR][currentC] = num;` 이 문장은 정확히 **$N^2 - 1$번 실행**된다.

1. 루프가 3개지만, 안쪽 루프들이 수행하는 총 횟수의 합이 $N^2$
2. 배열의 모든 칸($N \times N$)을 딱 한 번씩만 방문하므로 $O(N^2)$
